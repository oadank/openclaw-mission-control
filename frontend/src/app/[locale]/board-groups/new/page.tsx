...
export default function NewBoardGroupPage() {
  ...
  const t = useTranslations('createBoardGroup');
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (created.status !== 200) {
        throw new Error(t('errorUnableToCreate'));
      }
      ...
      >
        ...
        <div className="space-y-2">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <label className="text-sm font-medium text-slate-900">{t('boards')}</label>
            <span className="text-xs text-slate-500">
              {t('selected', { count: selectedBoardIds.size })}
            </span>
          </div>
          <Input
            value={boardSearch}
