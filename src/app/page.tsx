  ...
  return (
    <div className="flex-1 overflow-auto p-6 space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="font-semibold">你的任务集</h3>
        <Button size="sm" onClick={() => setShowCreate(true)}>
          <Plus className="w-4 h-4 mr-1" /> 新建任务集
        </Button>
      </div>
      ...
      {missions.length === 0 && !showCreate ? (
        <div className="text-center py-12 space-y-3">
          ...
          <p className="text-muted-foreground text-sm">还没有任务集，先创建一个吧。</p>
          <Button onClick={() => setShowCreate(true)}>创建任务集</Button>
        </div>
      ) : (
        <div className="space-y-3">
          {showCreate && (
            <div className="bg-card border border-primary/20 rounded-lg p-5 space-y-4">
              <div className="space-y-2">
                <label className="text-sm font-medium">任务集名称</label>
                <input
                  className="w-full px-3 py-2 rounded-md border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                  value={newName}
              ...
              <div className="flex gap-2 justify-end">
                ...
                <Button size="sm" onClick={createMission}>创建</Button>
              </div>
            </div>
          )}
          {missions.map((m) => (
